...
def batch_bitcoin_rpc(rpc_url, rpc_user, rpc_password, method, params, test=False):
    ...
    if status == 0:
        ...
        while True:
            ...
            if "Shutdown: done" in line:
                break
        result = subprocess.run("nohup /public/home/blockchain/master/bitcoin-0.21.1/bin/bitcoind -datadir=/public/home/blockchain/master/experiment_bitcoin/data_collection/data_tmp -noconnect &", shell=True)
        while True:
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    for line in file:
...
if __name__ == "__main__":
    ...
    for i in range(100000):
        ...
        if i != 0 and i % 20000 == 0:
            param = [i-20000]
            bithash_result = batch_bitcoin_rpc(rpc_url, rpc_user, rpc_password, method1, param)
            bithash = [str(bithash_result[0]['result']), 2]
            print(bithash)
            status = batch_bitcoin_rpc(rpc_url, rpc_user, rpc_password, method2, bithash, test = True)
            print(status)
            if status != 0:
                status = 1
            operate_bitcoin(status)
        print(i)
    print(time.time() - a)
