...
def batch_zcash_rpc(rpc_url, rpc_user, rpc_password, method, params_list,windows_memory):
    ...
    while True:
        try:
            ...
            #print(result)
            #print(result[0]['result']['height'])
            property_file = "/home/slave15/experiment/storage/zcash_data.properties"
            #print(123)
...
def zcash_to_graphson(params_list,windows_memory):
    ...
    print(str(file_num) + "start time: " + str(time.time()))
    # start to transform
    file_tx_dict = {}
    #print(raw_data)
    #print(len(blocks))
    ...
    with open(graphson_dir + current_json_file,'w') as fd:
        with open("/home/slave15/tmp/" + current_json_file,'w') as fm:
            ...
            while True:
                try:
                    property_file = "/home/slave15/experiment/storage/zcash_loader.properties"
                    content = file_property.parse(property_file)
            ...
            fd.close()
            #time.sleep(150)
            print(str(file_num) + "stop time: " + str(time.time()))
